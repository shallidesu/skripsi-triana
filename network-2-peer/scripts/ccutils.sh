...
function chaincodeInvokeInit() {
  ...
  # successln "Invoke transaction successful on Peer0 on channel '$CHANNEL_NAME'"
  parsePeerConnectionParameters $@
  res=$?
  verifyResult $res "Invoke transaction failed on channel '$CHANNEL_NAME' due to uneven number of peer and org parameters "
...
function chaincodeQuery() {
  # setGlobalsForPeer0
  # infoln "Querying on peer0.org1 on channel '$CHANNEL_NAME'..."
  # set -x
  ...
  # cat log.txt
  # verifyResult $res "Query result on Peer0 failed "
  # successln "Query successful on Peer0 on channel '$CHANNEL_NAME'"
  PEER=$1
  setGlobals $PEER
...
#   CHANNEL=$3
#   CC_NAME=$4
#   CC_INVOKE_CONSTRUCTOR=$5
  
#   infoln "Invoking on peer${PEER}.org${ORG} on channel '$CHANNEL_NAME'..."
#   local rc=1
#   local COUNTER=1
#   # continue to poll
#   # we either get a successful response, or reach MAX RETRY
#   while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
#     sleep $DELAY
#     infoln "Attempting to Invoke on peer${PEER}.org${ORG}, Retry after $DELAY seconds."
#     set -x
#     peer chaincode invoke -o localhost:7050 -C $CHANNEL_NAME -n ${CC_NAME} -c ${CC_INVOKE_CONSTRUCTOR} --tls --cafile $ORDERER_CA  --peerAddresses localhost:7051 --tlsRootCertFiles $PEER0_ORG1_CA --peerAddresses localhost:8051 --tlsRootCertFiles $PEER1_ORG1_CA --peerAddresses localhost:9051 --tlsRootCertFiles $PEER2_ORG1_CA  >&log.txt
#     res=$?
#     { set +x; } 2>/dev/null
#     let rc=$res
#     COUNTER=$(expr $COUNTER + 1)
...
#   cat log.txt
#   if test $rc -eq 0; then
#     successln "Invoke successful on peer${PEER}.org${ORG} on channel '$CHANNEL_NAME'"
#   else
#     fatalln "After $MAX_RETRY attempts, Invoke result on peer${PEER}.org${ORG} is INVALID!"
#   fi
# }
